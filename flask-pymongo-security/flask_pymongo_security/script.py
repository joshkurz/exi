...
class DeactivateUserCommand(_ToggleActiveCommand):
    ...
    def run(self, user_id):
        user = _datastore.user_model.find_one(dict(_id=ObjectId(user_id)))
        if user:
            if _datastore.deactivate_user(user):
                print "User '%s' has been deactivated" % user.vNam
        
        print "Failed to deactivate user with id: %s" % user_id
    ...
    def run(self, user_id):
        ...
        if user:
            if _datastore.activate_user(user):
                print "User '%s' has been activated" % user.vNam
                return
        
        print "Failed to active user with id: %s" % user_id
